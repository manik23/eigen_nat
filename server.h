...
MYSQL buildConnection(){
        ...
        if(!mysql_real_connect(&mysql,"localhost","root","#include","p2p",0,NULL,0))
        {
...
// Function to send udp request to the r client

int send_udp_conn_request_tor(int port,char *i_ip_address,char *mac_address,int udpSocket){
        ...
        if(result==NULL)
                ...
                cout<<"IP :"<<row[2]<<"   Port : "<<row[3]<<endl;
                   //create and send udp request to client i to make tcp connection
...
int listen_for_registration(int udpSocket){
        int  nBytes;
